...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '', 'checkout')) {
        // Token invalid – regenerate a new token and show error
        $_SESSION['error_message'] = 'Invalid security token. Please try again.';
        // Generate new token for the next attempt
        $csrf_token = generateCSRFToken('checkout');
        // Redirect to the same page to avoid reposting
        header('Location: checkout.php');
        exit;
    } else {
        // Token is valid – proceed with normal processing
